...
$(document).ready(function() {
	...
	console.log('Experienced developers sometimes use the shorthand $() for $( document ).ready()');
	w3.includeHTML();
});
...
$('.contactUsForm').on('submit', function() {
	...
	$.get({
		url: url,
		data: { name: name, email: email, address: address, city: city, phone: phone, message: message }
	})
		.done(function(data) {
			console.log('Data', data);
			console.log('Success', name, email, address, city, phone, message);

			$('.success-message').css('display', 'block');
			$('.stage').html('<p> Name: ' + name + '</p>');
			$('.stage').append('<p>Email : ' + email + '</p>');
			$('.stage').append('<p>Address: ' + address + '</p>');
